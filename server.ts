...
User Query: "${message}"`;
  ...
  if (isGeminiConfigured) {
    try {
      ...
      console.warn("Gemini API backend error, falling back to simulated high-fidelity responses:", error.message || error);
...
To reach **Level 3 (700+ Score)** and unlock premium benefits, I suggest checking your options in the **Growth Simulator**, or heading to the **Verification Center** to upload your pending internship offer letter.`;
      ...
      const cleanMsg = error.message || "Billing restriction or invalid key";
      const errorNote = `⚠️ *Note: The active Gemini key returned an API issue (${cleanMsg}). Showing simulated Coach response:* \n\n`;
      return res.json({ reply: errorNote + reply });
    }
  }
