...
async function startServer() {
  ...
  // API: Trust Coach AI chat handler proxies requests to Gemini 3.5
  app.post("/api/chat", async (req, res) => {
    ...
    // Check if Gemini API Key is missing or default
    if (!apiKey || apiKey === "MY_GEMINI_API_KEY" || apiKey.includes("REPLACE")) {
      console.warn("GEMINI_API_KEY not configured in Sandbox environment. Using Simulated Advisor mode.");
      
      // Fallback helper to provide deep high-fidelity smart simulation responses
      let reply = "";
...
This tells partner banks and loan gateways that you maintain very high self-discipline, reducing liability flags.`;
        ...
        reply = `Your summer Software Engineering internship iscurrently listed as **Pending**.
...
To reach **Level 3 (700+ Score)** and unlock premium benefits, I suggest checking your options in the **Growth Simulator**, or heading to the **Verification Center** to upload your pending internship offer letter.`;
    ...
    try {
      // Initialize model client with recommended telemetry
      ...
      // Construct a highly detailed system instructions setup
      ...
      // Construct prompt including details
...
User Query: "${message}"`;
      ...
      return res.status(500).json({ error: "Failed to connect to AI Gateway.", details: error.message });
    }
  });
